...
import ReverseEngineeringPanel from './ReverseEngineering/ReverseEngineeringPanel';
...
const Sidebar: React.FC<SidebarProps> = ({ onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'editor' | 'templates' | 'reverse'>('editor');
  ...
  const handleTabClick = (tab: 'editor' | 'templates' | 'reverse', event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handlePromptGenerated = (prompt: string) => {
    // Handle the generated prompt from reverse engineering
    console.log('Generated prompt:', prompt);
  };

  const handleThumbnailGenerated = (thumbnail: string) => {
    // Handle the generated thumbnail from face swap
  ...
  return (
    <aside className="h-screen overflow-y-auto bg-[#151823] border-r border-gray-800/50 flex flex-col" id="design-controls-sidebar">
      <div className="p-4 border-b border-gray-800/50 sticky top-0 bg-[#151823] z-10" data-section="header">
        ...
        >
          ...
          <button
            ...
            className={cn(
              "flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-[12px]",
              "font-medium transition-colors duration-200 relative z-10 cursor-pointer text-xs",
          ...
          <button
            data-tab="reverse"
            onClick={(e) => handleTabClick('reverse', e)}
            className={cn(
              "flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-[12px]",
              "font-medium transition-colors duration-200 relative z-10 cursor-pointer text-xs",
              activeTab === 'reverse' ? "text-white" : "text-[#A1A1AA] hover:text-white"
            )}
          >
            <Icon 
              ...
              className={cn(
                "h-4 w-4 transition-transform duration-200",
                activeTab === 'reverse' && "transform scale-[1.02]"
              )} 
            />
            <span>Reverse</span>
          </button>
      ...
      <div className="p-4 flex-1 overflow-y-auto" data-section="controls">
        {activeTab === 'reverse' ? (
